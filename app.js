...
app.post('/api/add-room', (req, res) => {
    ...
    db.query(sql, [roomNumber, roomType, pricePerNight], (err, result) => {
        if (err) {
            console.error('Error inserting room:', err);
            return res.status(500).json({ success: false, message: 'Error adding room to database.' });
        }
...
app.post('/api/reserve', (req, res) => {
   ...
   const userId = req.session.user.id; // Retrieve user ID from the session
    ...
    `;
    db.query(checkAvailabilitySql, [roomId, checkInDate, checkOutDate, checkInDate, checkOutDate], (err, results) => {
        if (err) {
        ...
        db.query(sql, [roomId], (err, room) => {
            ...
            // Insert reservation
            const insertSql = 'INSERT INTO reservations (user_id, room_id, check_in_date, check_out_date, total_price) VALUES (?, ?, ?, ?, ?)';
            db.query(insertSql, [userId, roomId, checkInDate, checkOutDate, totalPrice], (err, result) => {
                if (err) {
                    return res.status(500).json({ message: 'Error reserving room' });
                }

                // Optionally, update room availability
                const updateRoomAvailabilitySql = 'UPDATE rooms SET availability = ? WHERE id = ?';
                db.query(updateRoomAvailabilitySql, ['No', roomId], (err, updateResult) => {
                    if (err) {
